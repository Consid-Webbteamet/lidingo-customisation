...
- If the task explicitly asks for a local installed-copy sync, only sync built output and runtime files needed for verification. Do not sync Sass/CSS source files into `wp-content/plugins/lidingo-customisation/`. Keep all real source edits in this repo, and use Composer-based sync for deployment/prod.
...
- Prefer nested Sass selectors when they improve readability and keep selector relationships clear.
